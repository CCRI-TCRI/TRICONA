...
export function VotingBallot({ studentId, onVoteComplete }: VotingBallotProps) {
  ...
  const isPositionLocked = (index: number) => {
    // First position is never locked
    if (index === 0) return false
    // Position is locked if previous position doesn't have a vote
                      ...
                      <div
                        ...
                        className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-300 ${
                          isActive
                            ...
                            : isLocked
                            ? "border-gray-600 bg-gray-500/10 opacity-50 cursor-not-allowed"
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3 flex-1">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                              isActive ? "bg-blue-500" : hasVote ? "bg-green-500" : isLocked ? "bg-gray-600" : "bg-white/20"
                            }`}>
                              {hasVote ? (
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`font-medium truncate ${isActive ? "text-blue-300" : ""}`}>
                                {position.name}
                              </p>
